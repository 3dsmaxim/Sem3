...
не получается в середину воткнуть, 
загружать приходится последовательно так как конфлик строк происходит и нет функции или не известна чтобы вписать нужные строки куда надо с последующим смещение остальных строк
более менее адекватно мозно только последовательно подкгружать, при конфликте, счто у тебя написано кидает ниже строки вставки присодиняемой ветви.

Спасла перезагрузка ВСК, но папу потерял и пре открытии игнорировал ее наличие
пришлось папку переместить повыше.
